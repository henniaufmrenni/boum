...
const useDownloadMovie = async (
  session: Session,
  item: VideoMediaItem,
  playbackInfo: PlaybackInfom,
  selectedStorageLocation?: SelectedStorageLocation,
) => {
  const db = await getDBConnection();

  const itemDirectoryPath = path(item.Name, '', selectedStorageLocation);
  const imageLocation = path(item.Name, 'cover.jpg', selectedStorageLocation);
  ...
  // Create directory
  RNFS.mkdir(itemDirectoryPath).catch(err => {
  ...
  if (playbackInfoRes.MediaSources[0].TranscodingUrl !== undefined) {
    console.log('Transcoded download');
    return;
  } else {
    const tempPath = path(item.Name, item.Name, selectedStorageLocation);
    let permPath: string;
    ...
    RNFS.downloadFile({
      ...
      begin: async (res: DownloadBeginCallbackResult) => {
        ...
        createMovieEntry(db, item, permPath, album.Id, imageLocation);
      },
